...
u16 AD_Initial_Value[50];
u16 AD_Final_Value[5];

/** @brief	ADC输入端口初始化，PA0,PA1,PA2,PA3,PA4
  **/
void ADC_GPIO_Init(void)
{
	...
	GPIO_InitStruct.GPIO_Pin		=GPIO_Pin_0|GPIO_Pin_1|GPIO_Pin_2|GPIO_Pin_3|GPIO_Pin_4;//端口
...
void ADC1_In_Init(void)
{
	...
	//规则组输入通道选择，//ADC1,通道4，PA4,序列5，周期71.5
...
void ADC_DMA1_1_Init(void)
{
	...
	DMA_InitStruct.DMA_BufferSize					=50;															//传输次数
...
{
	AD_Final_Value[0]=exponential_moving_average(&AD_Initial_Value[0],10,5,0.9);
	AD_Final_Value[1]=exponential_moving_average(&AD_Initial_Value[1],10,5,0.9);
	AD_Final_Value[2]=exponential_moving_average(&AD_Initial_Value[2],10,5,0.9);
	AD_Final_Value[3]=exponential_moving_average(&AD_Initial_Value[3],10,5,0.9);
	
	AD_Final_Value[4]=exponential_moving_average(&AD_Initial_Value[4],10,5,0.9);
}
